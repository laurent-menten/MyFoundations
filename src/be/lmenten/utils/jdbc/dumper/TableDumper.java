...
public class TableDumper
{
	...
	public static void printTable( Connection con, String tableName, int maxRows )
	{
		printTable( con, tableName, maxRows, DEFAULT_MAX_TEXT_COL_WIDTH );
	}

	// -------------------------------------------------------------------------

	public static void printTable( Connection con, String tableName, int maxRows, int maxStringColWidth )
	{
		if( con == null )
		{
	...
	public static void printResultSet( ResultSet rs, int maxStringColWidth )
	{
		try
		{
			...
			for( int i = 1; i <= columnCount; i++ )
			{
				...
				c.setTypeCategory( ColumnCategory.whichCategory( c.getType() ) );
				columns.add( c );
			...
			while( rs.next() )
			{
				for( int i = 0 ; i < columnCount ; i++ )
				{
					...
					ColumnCategory category = c.getTypeCategory();
					String value;

					if( category == ColumnCategory.OTHER )
					{
						value = "(" + c.getTypeName() + ")";
					}
					else
